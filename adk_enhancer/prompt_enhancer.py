...
class PromptEnhancer:
    def __init__(self, prompt_id_var_prefix: str):
        ...
        self.agent_names_from_env = self.load_prompt_ids_from_env()
    ...
    def get_prompt_from_vertex_ai(self, agent_name: str) -> Optional[Prompt]:
        if agent_name not in self.agent_names_from_env.keys():
            return None
        prompt_id = self.agent_names_from_env[agent_name]
    ...
    def save_prompts(self):
        ...
        for agent_name, agent_definition in self.local_prompts.items():
            if agent_name in self.agent_names_from_env.keys():
